...
void SCD30_writeCommand(uint16_t command) {
    ...
    I2C_1_MasterWriteBuf(SCD30_I2C_ADDRESS, buf, 2, I2C_1_MODE_COMPLETE_XFER);
    for(;;){
...
void SCD30_writeCommandWithArguments(uint16_t command, uint16_t arguments) {
    static uint8_t checkSum, buf[5] = { 0 };
...
{
    I2C_1_MasterClearStatus();
    uint8_t status = I2C_1_MasterWriteBuf(SCD30_I2C_ADDRESS, data, len, I2C_1_MODE_COMPLETE_XFER);
    ...
    if(status == I2C_1_MSTR_NO_ERROR)
    {
        for(;;)
        {
            ...
            if(0 !=(I2C_1_MasterStatus() & I2C_1_MSTAT_WR_CMPLT))
            {
                break;
            }
        }
...
void SCD30_readBuffer(uint8_t* data, uint8_t len) 
{
    ...
    I2C_1_MasterReadBuf(SCD30_I2C_ADDRESS, data, len, I2C_1_MODE_COMPLETE_XFER);
    for(;;){
