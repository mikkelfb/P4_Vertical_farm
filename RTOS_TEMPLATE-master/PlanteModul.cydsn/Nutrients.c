...
void vTaskNutrientController()
{
    ...
    for(;;)
    {
        ...
        if(PHMessage.message > (fPHParameter*1.1))
        {
            bstate =0;
            xQueueSendToBack(xQueueNutrientPump[2], &bstate, portMAX_DELAY); 
        }
        ...
        {
            SW_UART_TEST_USB_PutString("ALARM EC");
            SW_UART_TEST_USB_PutString("\n");
        }       
