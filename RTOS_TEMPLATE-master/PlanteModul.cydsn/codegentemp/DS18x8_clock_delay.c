/*******************************************************************************
* File Name: DS18x8_clock_delay.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: DS18x8_clock_delay_Start
********************************************************************************
*
* Summary:
...
void DS18x8_clock_delay_Start(void) 
{
    ...
    DS18x8_clock_delay_CLKEN |= DS18x8_clock_delay_CLKEN_MASK;
	DS18x8_clock_delay_CLKSTBY |= DS18x8_clock_delay_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: DS18x8_clock_delay_Stop
********************************************************************************
*
* Summary:
...
void DS18x8_clock_delay_Stop(void) 
{
    ...
    DS18x8_clock_delay_CLKEN &= (uint8)(~DS18x8_clock_delay_CLKEN_MASK);
	DS18x8_clock_delay_CLKSTBY &= (uint8)(~DS18x8_clock_delay_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: DS18x8_clock_delay_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((DS18x8_clock_delay_CLKEN & DS18x8_clock_delay_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = DS18x8_clock_delay_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = DS18x8_clock_delay_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* DS18x8_clock_delay__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(DS18x8_clock_delay_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(DS18x8_clock_delay_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: DS18x8_clock_delay_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: DS18x8_clock_delay_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = DS18x8_clock_delay_GetSourceRegister();
    uint16 oldDivider = DS18x8_clock_delay_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = DS18x8_clock_delay_CLKEN & DS18x8_clock_delay_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(DS18x8_clock_delay_DIV_PTR, clkDivider);
                DS18x8_clock_delay_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                DS18x8_clock_delay_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(DS18x8_clock_delay_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((DS18x8_clock_delay_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(DS18x8_clock_delay_DIV_PTR, clkDivider);
				DS18x8_clock_delay_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: DS18x8_clock_delay_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(DS18x8_clock_delay_DIV_PTR);
}


/*******************************************************************************
* Function Name: DS18x8_clock_delay_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    DS18x8_clock_delay_MOD_SRC |= modeBitMask & (uint8)DS18x8_clock_delay_MODE_MASK;
}


/*******************************************************************************
* Function Name: DS18x8_clock_delay_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    DS18x8_clock_delay_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(DS18x8_clock_delay_MODE_MASK));
}


/*******************************************************************************
* Function Name: DS18x8_clock_delay_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return DS18x8_clock_delay_MOD_SRC & (uint8)(DS18x8_clock_delay_MODE_MASK);
}


/*******************************************************************************
* Function Name: DS18x8_clock_delay_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = DS18x8_clock_delay_GetDividerRegister();
    uint8 oldSrc = DS18x8_clock_delay_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        DS18x8_clock_delay_MOD_SRC |= CYCLK_SSS;
        DS18x8_clock_delay_MOD_SRC =
            (DS18x8_clock_delay_MOD_SRC & (uint8)(~DS18x8_clock_delay_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: DS18x8_clock_delay_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return DS18x8_clock_delay_MOD_SRC & DS18x8_clock_delay_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: DS18x8_clock_delay_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    DS18x8_clock_delay_PHASE = clkPhase & DS18x8_clock_delay_PHASE_MASK;
}


/*******************************************************************************
* Function Name: DS18x8_clock_delay_GetPhase
********************************************************************************
*
* Summary:
...
{
    return DS18x8_clock_delay_PHASE & DS18x8_clock_delay_PHASE_MASK;
}
