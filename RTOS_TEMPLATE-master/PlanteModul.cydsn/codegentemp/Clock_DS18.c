...
/*******************************************************************************
* Function Name: Clock_DS18_Start
********************************************************************************
*
* Summary:
...
void Clock_DS18_Start(void) 
{
    ...
    Clock_DS18_CLKEN |= Clock_DS18_CLKEN_MASK;
	Clock_DS18_CLKSTBY |= Clock_DS18_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_DS18_Stop
********************************************************************************
*
* Summary:
...
void Clock_DS18_Stop(void) 
{
    ...
    Clock_DS18_CLKEN &= (uint8)(~Clock_DS18_CLKEN_MASK);
	Clock_DS18_CLKSTBY &= (uint8)(~Clock_DS18_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_DS18_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_DS18_CLKEN & Clock_DS18_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_DS18_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_DS18_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_DS18__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_DS18_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_DS18_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_DS18_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_DS18_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_DS18_GetSourceRegister();
    uint16 oldDivider = Clock_DS18_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_DS18_CLKEN & Clock_DS18_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_DS18_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_DS18_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_DS18_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_DS18_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_DS18_MOD_SRC |= modeBitMask & (uint8)Clock_DS18_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_DS18_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_DS18_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_DS18_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_DS18_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_DS18_MOD_SRC & (uint8)(Clock_DS18_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_DS18_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_DS18_GetDividerRegister();
    uint8 oldSrc = Clock_DS18_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_DS18_MOD_SRC |= CYCLK_SSS;
        Clock_DS18_MOD_SRC =
            (Clock_DS18_MOD_SRC & (uint8)(~Clock_DS18_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_DS18_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_DS18_MOD_SRC & Clock_DS18_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_DS18_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_DS18_PHASE = clkPhase & Clock_DS18_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_DS18_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_DS18_PHASE & Clock_DS18_PHASE_MASK;
}
