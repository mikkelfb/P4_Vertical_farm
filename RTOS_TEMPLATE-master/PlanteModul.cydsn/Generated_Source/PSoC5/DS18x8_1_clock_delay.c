/*******************************************************************************
* File Name: DS18x8_1_clock_delay.c
* Version 2.20
*
*  Description:
...
#include "DS18x8_1_clock_delay.h"
...
/*******************************************************************************
* Function Name: DS18x8_1_clock_delay_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void DS18x8_1_clock_delay_Start(void) 
{
    /* Set the bit to enable the clock. */
    DS18x8_1_clock_delay_CLKEN |= DS18x8_1_clock_delay_CLKEN_MASK;
	DS18x8_1_clock_delay_CLKSTBY |= DS18x8_1_clock_delay_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: DS18x8_1_clock_delay_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void DS18x8_1_clock_delay_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    DS18x8_1_clock_delay_CLKEN &= (uint8)(~DS18x8_1_clock_delay_CLKEN_MASK);
	DS18x8_1_clock_delay_CLKSTBY &= (uint8)(~DS18x8_1_clock_delay_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: DS18x8_1_clock_delay_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void DS18x8_1_clock_delay_StopBlock(void) 
{
    if ((DS18x8_1_clock_delay_CLKEN & DS18x8_1_clock_delay_CLKEN_MASK) != 0u)
    {
...
#if defined(DS18x8_1_clock_delay__CFG3)
        CLK_DIST_AMASK = DS18x8_1_clock_delay_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = DS18x8_1_clock_delay_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* DS18x8_1_clock_delay__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(DS18x8_1_clock_delay_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(DS18x8_1_clock_delay_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: DS18x8_1_clock_delay_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void DS18x8_1_clock_delay_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        DS18x8_1_clock_delay_CLKSTBY &= (uint8)(~DS18x8_1_clock_delay_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: DS18x8_1_clock_delay_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void DS18x8_1_clock_delay_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = DS18x8_1_clock_delay_GetSourceRegister();
    uint16 oldDivider = DS18x8_1_clock_delay_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = DS18x8_1_clock_delay_CLKEN & DS18x8_1_clock_delay_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(DS18x8_1_clock_delay_DIV_PTR, clkDivider);
                DS18x8_1_clock_delay_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                DS18x8_1_clock_delay_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(DS18x8_1_clock_delay_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((DS18x8_1_clock_delay_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(DS18x8_1_clock_delay_DIV_PTR, clkDivider);
				DS18x8_1_clock_delay_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: DS18x8_1_clock_delay_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 DS18x8_1_clock_delay_GetDividerRegister(void) 
{
    return CY_GET_REG16(DS18x8_1_clock_delay_DIV_PTR);
}


/*******************************************************************************
* Function Name: DS18x8_1_clock_delay_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void DS18x8_1_clock_delay_SetModeRegister(uint8 modeBitMask) 
{
    DS18x8_1_clock_delay_MOD_SRC |= modeBitMask & (uint8)DS18x8_1_clock_delay_MODE_MASK;
}


/*******************************************************************************
* Function Name: DS18x8_1_clock_delay_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void DS18x8_1_clock_delay_ClearModeRegister(uint8 modeBitMask) 
{
    DS18x8_1_clock_delay_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(DS18x8_1_clock_delay_MODE_MASK));
}


/*******************************************************************************
* Function Name: DS18x8_1_clock_delay_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 DS18x8_1_clock_delay_GetModeRegister(void) 
{
    return DS18x8_1_clock_delay_MOD_SRC & (uint8)(DS18x8_1_clock_delay_MODE_MASK);
}


/*******************************************************************************
* Function Name: DS18x8_1_clock_delay_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void DS18x8_1_clock_delay_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = DS18x8_1_clock_delay_GetDividerRegister();
    uint8 oldSrc = DS18x8_1_clock_delay_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        DS18x8_1_clock_delay_MOD_SRC |= CYCLK_SSS;
        DS18x8_1_clock_delay_MOD_SRC =
            (DS18x8_1_clock_delay_MOD_SRC & (uint8)(~DS18x8_1_clock_delay_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: DS18x8_1_clock_delay_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 DS18x8_1_clock_delay_GetSourceRegister(void) 
{
    return DS18x8_1_clock_delay_MOD_SRC & DS18x8_1_clock_delay_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: DS18x8_1_clock_delay_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void DS18x8_1_clock_delay_SetPhaseRegister(uint8 clkPhase) 
{
    DS18x8_1_clock_delay_PHASE = clkPhase & DS18x8_1_clock_delay_PHASE_MASK;
}


/*******************************************************************************
* Function Name: DS18x8_1_clock_delay_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 DS18x8_1_clock_delay_GetPhaseRegister(void) 
{
    return DS18x8_1_clock_delay_PHASE & DS18x8_1_clock_delay_PHASE_MASK;
}
